...
cm = inch / 2.54;
    ...
    fig = Figure(size = (5cm, 4cm))
    
    # Panel izquierdo: función abs(x)/x
    ax1 = Axis(fig[1, 1], 
               title = L"f(x) = \frac{|x|}{x}",
               xlabel = L"x", 
               ylabel = L"f(x)",
               limits = (-3, 3, -1.5, 1.5),
               xticks = -3:1:3,
    ...
    lines!(ax1, x_left, fill(-1.0, length(x_left)), color=:blue, linewidth=2)
    lines!(ax1, x_right, fill(1.0, length(x_right)), color=:blue, linewidth=2)
    
    # Añadir huecos en la discontinuidad
    scatter!(ax1, [0.0], [1.0], color=:white, markersize=15, 
...
cm = inch / 2.54;
    ...
    ax2 = Axis(fig[1, 1],
               title = L"f(x) = \frac{x-2}{2(x-2)}(x^2+x)",
               xlabel = "x",
               ylabel = "f(x)",
               limits = (-1, 5, 0, 12),
               xticks = -1:1:5,
    ...
    x_left = range(-1, 1.99, length=100)
    ...
    scatter!(ax2, [2.0], [f(2.0)], color=:white, markersize=20,
             strokecolor=:blue, strokewidth=2, marker=:circle)
